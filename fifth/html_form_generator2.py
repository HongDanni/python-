...
def generate_webform(text_field_list=[], checkbox_field_list=[]):
    generated_fields = "\n".join(
        map(lambda x: "{0}<br><input type='text' name={0}><br>".format(x), text_field_list)
    )

    generated_fields += "\n".join(
        map(lambda x: "<label><input type='checkbox' id='{0}' value='{0}'>{0}<br>".format(x), checkbox_field_list)
    )
...
def build_html_form(text_field_list=[], checkbox_field_list=[]):
    with open('form_file2.html', 'w') as f:
        f.write(
